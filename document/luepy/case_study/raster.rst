Raster
======
A raster is a 2-dimensional grid of cells, where each cell represents a square piece of the space and contains a value. Rasters are often used to capture the magnitude of some continuous varying field, like topographic elevation, or surface temperature.
...
.. note::
    ...
    There is a somewhat subtle difference between a space box containing a 2D array of values and a space box containing a 2D discretized value. In the first case, the 2D array is representative for the whole space box, while in the latter case, each individual cell value in the 2D array is mapped to a rectangular cell in the space box. Whether or not the value is discretized is what makes these two kinds of data different.
