Constant
========
A constant is a single value that does not change through time and space. Whether a value can be considered constant or not is often dependent on the extent of the time and space that are considered, and on the accuracy required by the application. Given a small time and space extent some value may not vary, but it may vary over larger time and space extents. Also, one application may not be sensitive to some value's variation, while another may required detailed information about the variation of the value through time and/or space.

An example of a value that is often considered constant is the standard gravity (at least near the surface of the earth). Examples of values that are always considered constant are the physical constants like the speed of light and the elementary charge.
