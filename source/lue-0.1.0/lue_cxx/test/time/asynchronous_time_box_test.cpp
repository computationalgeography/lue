#define BOOST_TEST_MODULE lue time asynchronous_time_box
#include <boost/test/unit_test.hpp>
#include "lue/time/asynchronous_time_box.hpp"
...
BOOST_AUTO_TEST_CASE(create_collection)
{
    ...
    auto collection = create_asynchronous_time_box(file, clock);

    BOOST_CHECK_EQUAL(collection.nr_items(), 0);

    hdf5::Shape const value_shape{2};
    ...
    for(hsize_t i = 0; i < nr_items; ++i) {
        ...
        values[i].resize(nr_time_domain_items_ * 2);
    ...
    for(hsize_t i = 0; i < nr_items; ++i) {
        ...
        values_read[i].resize(nr_time_domain_items_ * 2);
    ...
    // Write and read value for item with idx 2
    {
        ...
        std::vector<time::DurationCount> new_value(nr_time_domain_items_ * 2);
        std::iota(new_value.begin(), new_value.end(), 99);
        collection.write(item_idx, new_value.data());

        std::vector<time::DurationCount> value_read(nr_time_domain_items_ * 2);
