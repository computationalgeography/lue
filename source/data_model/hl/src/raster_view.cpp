...

        {
            ...
            // Open space discretization property and read information
            {
                ...
                auto& space_discretization_property =
                    properties
                        .template collection<same_shape::Properties>()[space_discretization_property_name];
                    ...
                    if (properties.contains(space_discretization_property_name))
                    {
                                ...
                                ValueVariability::constant)
                        {
                            ...
                            auto const& discretization_property{properties.collection<
                                same_shape::Properties>()[space_discretization_property_name]};
        ...
        namespace constant {
            ...
            */
                ...
                std::string const& layer_name)
            {
                ...
                auto raster_view{
                    open_raster_view(std::move(dataset_ptr), phenomenon_name, property_set_name)};
                auto layer{raster_view.layer(layer_name)};

                return std::make_tuple(
                    raster_view.object_id(), raster_view.grid_shape(), layer.file_datatype());
            }
        ...
        namespace variable {
            ...
            */
...

            {
                ...
                auto& time_discretization_property = properties.collection<
                ...
                std::string const& name, hdf5::Datatype const& datatype)
            {
                ...
                std::copy(
                    this->grid_shape().begin(), this->grid_shape().end(), object_array_shape.begin() + 1);
                        ...
                        if (properties.contains(time_discretization_property_name))
                        {
                                    ...
                                    ShapeVariability::constant)
                            {
                                ...
                                if (!discretization_property.time_is_discretized() &&
                                    ...
                                    value.nr_arrays() == 1 && value.array_shape() == hdf5::Shape{1} &&
                                    value.file_datatype() ==
                                        hdf5::Datatype{
                                            hdf5::StandardDatatypeTraits<hdf5::Shape::value_type>::type_id()})
                                // value.file_datatype() ==
