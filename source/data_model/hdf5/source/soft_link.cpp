...
namespace lue {
    namespace hdf5 {
        ...
        SoftLink::SoftLink(Group const& group, std::string const& name):
        ...
        {
            return link_exists(group.id(), name) && link_type(group.id(), name) == ::H5L_TYPE_SOFT;
        }


        SoftLink create_soft_link(Group& group, Identifier const& target, std::string const& name)
        {
            ::herr_t const status{::H5Lcreate_soft(
                target.pathname().c_str(), group.id(), name.c_str(), H5P_DEFAULT, H5P_DEFAULT)};
