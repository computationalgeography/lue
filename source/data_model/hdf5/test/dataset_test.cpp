...
class Read2DDatasetFixture
{
    ...
    public:
...

        {
            ...
            dataset = std::make_unique<lh5::Dataset>(lh5::create_dataset(
                file.id(), dataset_name, file_datatype, dataspace, creation_property_list));
        ...
        void test_read_row(std::size_t const row)
        {
            ...
            dataset->read(memory_datatype, lh5::Hyperslab(start, stride, count), values_read.data());
        ...
        void test_read_col(std::size_t const col)
        {
        ...
        void test_read_cell(std::size_t const row, std::size_t const col)
        {
            ...
            dataset->read(memory_datatype, lh5::Hyperslab(start, stride, count), &value_read);
...
BOOST_AUTO_TEST_CASE(dataset_utf8)
{
    ...
    char const* values[nr_rows][nr_cols] = {
        {"ôû=€", "-111111111111111111111111111"}, {"", "9"}, {"ñahè", "ßßß"}};

    {
        ...
        auto dataset =
            lh5::create_dataset(file.id(), dataset_name, file_datatype, dataspace, creation_property_list);
...

    {
        ...
        lh5::VLenMemory vlen{memory_datatype, dataset.dataspace(), values_read};
        ...
        for (std::size_t r = 0; r < nr_rows; ++r)
        {
            for (std::size_t c = 0; c < nr_cols; ++c)
            {
                ...
                BOOST_CHECK_EQUAL(std::string{values[r][c]}, std::string{values_read[r][c]});
            }
        }
    }
}
