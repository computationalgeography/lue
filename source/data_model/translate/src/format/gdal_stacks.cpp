...
GDALStacks::GDALStacks(
    std::vector<std::string> const& dataset_names)
    ...
    if(!_stacks.empty()) {
        if(domains.size() != 1) {
            throw std::runtime_error(
                "Domains of raster stacks must be equal, but they are not");
        }

        if(discretizations.size() != 1) {
            throw std::runtime_error(
                "Discretizations of raster stacks must be equal, but they are "
                "not");
        }
