...
namespace lue {
    namespace utility {
        namespace {
            ...
            bool has_extension(std::string const& pathname)
            {
                return std::filesystem::path(pathname).has_extension();
            }


            [[maybe_unused]] std::string extension(std::string const& pathname)
            {
                return std::filesystem::path(pathname).extension().string();
            }


            std::string stem(std::string const& pathname)
            {
                ...
                data_model::MobileSpacePoint const& lue_space_point)
            {
                ...
                std::size_t max_size_active_set = 0;
                {
                    ...
                    std::transform(
                        ...
                        [](auto const idx1, auto const idx2) { return idx2 - idx1; });
                    max_size_active_set = *std::max_element(set_sizes.begin(), set_sizes.end());
                }
