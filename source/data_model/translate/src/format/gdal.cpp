...
///         //         }
///         //
///         //         gdal::Raster gdal_raster{std::move(gdal_dataset)};
///         //
...
///         namespace {
///
///             GDALDataType memory_datatype_to_gdal_datatype(hdf5::Datatype const& datatype)
///             {
///                 GDALDataType result{GDT_Unknown};
///
...
///                     default:
///                     {
///                         throw std::runtime_error("Cannot map GDAL datatype to LUE datatype");
///                     }
///                 }
///
///                 return result;
...
///             auto dataset = GDALDatasetPtr(
///                 driver.Create(dataset_name.c_str(), shape[1], shape[0], nr_bands, datatype, nullptr),
///                 GDALDatasetDeleter{});
///
///             if (!dataset)
///             {
...
///         {
///             return create_dataset(*gdal_driver_by_name(driver_name), dataset_name, shape, nr_bands,
///             datatype);
///         }
///
///
///         GDALDatasetPtr create_dataset(GDALDriver& driver, std::string const& dataset_name)
///         {
///             return create_dataset(driver, dataset_name, hdf5::Shape{0, 0}, 0, GDT_Unknown);
///         }
///
///
///         GDALDatasetPtr create_dataset(
...
///                 driver, dataset_name, shape, nr_bands, memory_datatype_to_gdal_datatype(datatype));
///         }
///
///
///         GDALDatasetPtr create_dataset(
...
///             auto dataset = create_dataset(*driver, dataset_name, shape, nr_bands, datatype);
///
///             return dataset;
///         }
///
...
///
///
///             GDALBlock natural_blocks(GDALRasterBand& raster_band)
///             {
///                 int block_size_x{};
///                 int block_size_y{};
///
...
///                 int const block_x, int const block_y, void* data, GDALRasterBand& raster_band)
///             {
...
///
///                         hdf5::Count const count = {1, 1, nr_valid_cells_y, nr_valid_cells_x};
///
///                         hdf5::Hyperslab const hyperslab{offset, count};
///
...
///                 if (memory_datatype == hdf5::native_uint8)
///                 {
///                     write_raster_band<std::uint8_t>(value, time_point_idx, time_step_idx, raster_band);
///                 }
///                 else if (memory_datatype == hdf5::native_uint16)
///                 {
///                     write_raster_band<std::uint16_t>(value, time_point_idx, time_step_idx, raster_band);
///                 }
///                 else if (memory_datatype == hdf5::native_int16)
///                 {
///                     write_raster_band<std::int16_t>(value, time_point_idx, time_step_idx, raster_band);
///                 }
///                 else if (memory_datatype == hdf5::native_uint32)
///                 {
///                     write_raster_band<std::uint32_t>(value, time_point_idx, time_step_idx, raster_band);
///                 }
///                 else if (memory_datatype == hdf5::native_int32)
///                 {
///                     write_raster_band<std::int32_t>(value, time_point_idx, time_step_idx, raster_band);
///                 }
///                 else if (memory_datatype == hdf5::native_uint64)
///                 {
///                     write_raster_band<std::uint64_t>(value, time_point_idx, time_step_idx, raster_band);
///                 }
///                 else if (memory_datatype == hdf5::native_int64)
///                 {
///                     write_raster_band<std::int64_t>(value, time_point_idx, time_step_idx, raster_band);
///                 }
///                 else if (memory_datatype == hdf5::native_float32)
///                 {
///                     write_raster_band<float>(value, time_point_idx, time_step_idx, raster_band);
///                 }
///                 else if (memory_datatype == hdf5::native_float64)
///                 {
///                     write_raster_band<double>(value, time_point_idx, time_step_idx, raster_band);
///                 }
///                 else
///                 {
///                     throw std::runtime_error("Cannot write value with this datatype to a GDAL raster
///                     band");
///                 }
///             }
///
///
///             template<typename T>
///             void write_raster_band(data_model::Array const& array, GDALRasterBand& raster_band)
///             {
///                 // It is assumed here that array only contains a 2D array
///                 auto const blocks = natural_blocks(raster_band);
///
...
///
///                         hdf5::Offset offset = {
///                             block_y * blocks.block_size_y(), block_x * blocks.block_size_x()};
///
///                         hdf5::Count const count = {nr_valid_cells_y, nr_valid_cells_x};
///
///                         hdf5::Hyperslab const hyperslab{offset, count};
///
///                         array.read(memory_datatype, hyperslab, values.data());
...
///
///
///         GDALDatasetPtr create_gdal_dataset(std::string const& driver_name, std::string const&
///         dataset_name)
///         {
///             auto* driver = gdal_driver_by_name(driver_name);
...
///
///                 auto gdal_dataset = create_dataset(
///                     "GTiff", raster_name, raster_view.grid_shape(), nr_bands, layer.memory_datatype());
///
///                 // FIXME
///                 double const cell_size{0.000992063492063};
///                 double const west{space_box[0]};
///                 double const north{space_box[3]};
///                 std::array<double, 6> geo_transform{west, cell_size, 0, north, 0, -cell_size};
///
...
///                         "GTiff",
///                         stack_name[time_step],
///                         raster_view.grid_shape(),
///                         nr_bands,
///                         layer.memory_datatype());
///
///                     // FIXME
///                     double const cell_size{0.000992063492063};
///                     double const west{space_box[0]};
...
///                     // CPLFree( pszSRS_WKT );
///
///                     write_raster_band(layer, time_point_idx, time_step, *gdal_dataset->GetRasterBand(1));
///                 }
///             }
///             else
///             {
///                 throw std::runtime_error(fmt::format(
///                     "No raster named {} found in property_set {}", property_name, property_set_name));
///             }
///
///
///             ///     // Find requested information in LUE dataset -------------------------------
