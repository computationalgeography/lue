...
namespace lue {
    namespace utility {
        namespace {
            ...
            class GeoEASHeader
            {
                ...
                public:
                    ...
                    GeoEASHeader(std::string const& title, Variables const& variables):
                        _title{title},
            ...
            class ColumnDefinition
            {
                ...
                public:
                    ...
                    explicit ColumnDefinition(std::string const& value_type_name):
            ...
            class TableDefinition
            {
                ...
                public:
                    ...
                    TableDefinition(ColumnDefinitions const& column_definitions, char const separator):
            ...
            class Table
            {
                ...
                public:
                    ...
                    Table(TableDefinition const& definition, Columns&& columns):
            ...
            bool parse_header(Iterator& first, Iterator last, GeoEASHeader& header)
            {
                ...
                status = qi::parse(first, last, qi::repeat(nr_variables)[rule >> qi::eol], variables);
            ...
            bool token_is_int(std::string const& token)
            {
                namespace qi = boost::spirit::qi;
            ...
            bool token_is_double(std::string const& token)
            {
                namespace qi = boost::spirit::qi;
            ...
            template<typename Iterator>
            bool parse_table_definition(Iterator first, Iterator last, TableDefinition& table_definition)
            {
            ...
            template<typename Iterator>
            bool parse_table(Iterator& first, Iterator last, TableDefinition const& definition, Table& table)
            {
                namespace qi = boost::spirit::qi;
                namespace phoenix = boost::phoenix;
                ...
                {
                    row_parser = row_parser.copy() >> +qi::blank >> column_parsers[i].copy();
                }
            ...
            std::string const& geo_eas_pathname, std::string const& lue_pathname, Metadata const& metadata)
        {
            ...
            {
                assert(table_definition.column_definitions()[idx].value_type_name() == "double");
            }
            ...
            std::string const geo_eas_dataset_name = std::filesystem::path(geo_eas_pathname).stem().string();
            std::string phenomenon_name = geo_eas_dataset_name;
            std::string property_set_name = "point";

            auto const& root_json = metadata.object();

            auto const dataset_json =
                json::object(root_json, JSONPointer{"/datasets"}, "name", geo_eas_dataset_name);
            ...
            if (json::has_key(dataset_json, "phenomenon"))
            {
                ...
                {
                    property_set_name =
                        json::string(phenomenon_json, JSON::json_pointer("/property_set/name"));
                }
            }
            ...
            if (nr_points != nr_items)
            {
                throw std::runtime_error(boost::str(
                    boost::format("Number of space points in meta-data file (%1%) must equal the "
            ...
            auto const property_name = json::string(dataset_json, JSONPointer{"/property/name"});

            hl::TimeSeriesDomain const time_domain{clock, start_time_point, end_time_point};
