...
namespace lue::utility {
    namespace {
        ...
        auto has_extension(std::string const& pathname) -> bool
        {
            return std::filesystem::path(pathname).has_extension();
        }


        [[maybe_unused]] auto extension(std::string const& pathname) -> std::string
        {
            return std::filesystem::path(pathname).extension().string();
        }


        auto stem(std::string const& pathname) -> std::string
        {
        ...
        {
            auto* layer = gdal_dataset.CreateLayer(layer_name.c_str(), nullptr, wkbPoint, nullptr);
            ...
            {
                auto feature =
                    OGRFeaturePtr(OGRFeature::CreateFeature(layer->GetLayerDefn()), OGRFeatureDeleter{});
                ...
                {
                    throw std::runtime_error("Cannot write feature to layer " + layer_name);
                }
            }
        }
        ...
        {
            auto* layer = gdal_dataset.CreateLayer(layer_name.c_str(), nullptr, wkbPolygon, nullptr);
            ...
            data_model::MobileSpacePoint const& lue_space_point)
        {
            ...
            data_model::ActiveSetIndex const& lue_active_set_idx = lue_object_tracker.active_set_index();
            data_model::ActiveObjectID const& lue_active_object_id = lue_object_tracker.active_object_id();
            ...
            std::size_t max_size_active_set = 0;
            {
                ...
                std::transform(
                    set_idxs.begin(),
                    set_idxs.end() - 1,    // First input range
                    set_idxs.begin() + 1,  // Second input range
                    set_sizes.begin(),     // Destination range
                    [](auto const idx1, auto const idx2) -> auto { return idx2 - idx1; });
                max_size_active_set = *std::ranges::max_element(set_sizes);
            }
            ...
            for (data_model::Count cell_idx = 0; cell_idx < nr_time_cells; ++cell_idx)
            {

                // Figure out which space points are part of the active set at
                // time cell cell_idx
                next_set_idx = set_idxs[cell_idx + 1];
                ...
                layer_name = std::format("{}-{}", property_set_name, cell_idx + 1);
        ...
        data_model::Dataset& dataset, std::string const& shapefile_name, Metadata const& metadata)
    {
        ...
        auto const lue_dataset_name = std::filesystem::path(dataset.pathname()).stem().string();
        ...
        auto const space_domain_item_type = space_configuration.value<data_model::SpaceDomainItemType>();
                ...
                switch (space_domain_item_type)
                {
                    ...
                    case data_model::SpaceDomainItemType::box:
                    {
                        ...
                        auto stationary_space_box =
                            space_domain.value<data_model::StationarySpaceBox>(hdf5::native_float64);
                        ...
                        if (!write_properties)
                        {
                            write_shapefile(shapefile_name, property_set_name, stationary_space_box);
                        }
                        else
                        {

                            data_model::TimeDomain& time_domain = property_set.time_domain();
                            auto const& time_configuration = time_domain.configuration();
            ...
            case data_model::Mobility::mobile:
            {
                ...
                switch (space_domain_item_type)
                {
                    ...
                    case data_model::SpaceDomainItemType::point:
                    {
                        ...
                        // GDAL coordinates are 64-bit floats
                        auto mobile_space_point =
                            space_domain.value<data_model::MobileSpacePoint>(hdf5::native_float64);
                        ...
                        switch (time_domain_item_type)
                        {
                            ...
                            case data_model::TimeDomainItemType::cell:
                            {
                                ...
                                auto const time_cell = time_domain.value<data_model::TimeCell>();
