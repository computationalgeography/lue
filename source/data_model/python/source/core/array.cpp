...
namespace lue::data_model {
    ...
    */
    ...
    static auto create_strides(hdf5::Shape const& shape, std::size_t size_of_element) -> std::vector<size_t>
    {
        assert(!shape.empty());
        ...
        std::ranges::transform(
            std::ranges::reverse_view(shape),
            strides.begin(),
            [&stride](std::size_t const& v) -> size_t
            {
        ...
        std::ranges::transform(
            strides,
            strides.begin(),
            [size_of_element](std::size_t const& v) -> std::size_t { return v * size_of_element; });
    ...
    template<typename T>
    auto create_array(hdf5::Shape const& shape) -> py::array
    {
        // assert(!shape.empty());
    ...
    static auto create_array(
        Array const& array, hdf5::Hyperslab const& hyperslab, hdf5::Shape const& slice_shape) -> py::array
    {
        assert(!slice_shape.empty());
    ...
    template<typename T>
    auto create_scalar(Array const& array, hdf5::Hyperslab const& hyperslab) -> py::object
    {
        T value;
    ...
    static auto create_scalar(Array const& array, hdf5::Hyperslab const& hyperslab) -> py::object
    {
    ...
    static auto read_hyperslab_from_array(
        Array const& array, hdf5::Hyperslab const& hyperslab, hdf5::Shape const& slice_shape) -> py::object
    {
    ...
    static auto create_hyperslab(hdf5::Shape const shape, std::vector<py::object> const& indices)
        -> std::tuple<hdf5::Hyperslab, hdf5::Shape>
    {
        ...
        for (size_t i = 0; i < indices.size(); ++i)
        {
            ...
            try
            {
                [[maybe_unused]] auto string = py::cast<std::string>(index);
    ...
    TODO
        ...
        py::class_<Array, hdf5::Dataset>(module, "Array", "Array docstring...")
            ...
            .def(
                ...
                { throw std::invalid_argument("on the fly iteration not supported, use the numpy array"); })
            ...
            .def(
                "__setitem__",
                [](Array& array, std::int64_t const index, py::int_ const& value) -> void
                { set_item(array, index, std::int64_t{value}); })

            .def(
                "__setitem__",
                [](Array& array, std::int64_t const index, py::float_ const& value) -> void
            ...
            .def(
                "__setitem__",
                [](Array& array, py::slice const& slice, py::int_ const& value) -> void
                { set_item(array, slice, std::int64_t{value}); })

            .def(
                "__setitem__",
                [](Array& array, py::slice const& slice, py::float_ const& value) -> void
...
#define SETITEM_INTEGER(T)                                                                                   \
    .def(                                                                                                    \
        ...
        [](Array& array,                                                                                     \
           std::int64_t const index,                                                                         \
           py::array_t<T, py::array::c_style /* | py::array::forcecast */>& values) -> auto                  \
...
#undef SETITEM_INDICES
            ...
            .def(
                "__setitem__",
                [](Array& array, py::slice const& slice, py::array& values) -> void
                {
                    ...
                    if (utf8_encoded_strings)
                    {
                        ...
                        for (std::size_t i = 0; i < nr_strings; ++i)
                        {
                            ...
                            std::ranges::copy(string_utf8, hdf5_string);
                    ...
                    else
                    {
                        ...
                        {
                            auto const* py_buffer_utf32 = static_cast<Py_UCS4 const*>(info.ptr);
                            ...
                            for (std::size_t i = 0; i < nr_strings; ++i)
                            {
                                ...
                                auto py_string_utf8 = py::reinterpret_steal<py::str>(
                                ...
                                auto string_utf8 = py_string_utf8.cast<std::string>();
                                boost::algorithm::trim_right_if(
                                    string_utf8, [](char const c) -> bool { return c == '\0'; });
