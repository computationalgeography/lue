...
namespace lue {
    namespace data_model {
        namespace {
            ...
            template<typename PropertiesSpecialization>
            std::string join_property_names(Properties const& properties)
            {
                return boost::algorithm::join(properties.names<PropertiesSpecialization>(), ", ");
            }


            std::vector<std::string>& join_collections(
                std::vector<std::string> const& from_collection, std::vector<std::string>& to_collection)
            {
                std::copy(from_collection.begin(), from_collection.end(), std::back_inserter(to_collection));

                return to_collection;
            }


            static std::string informal_string_representation(PropertySet const& property_set)
            {
            ...
            std::string const& name)
        {
            ...
            auto const shape_per_object = properties.shape_per_object(name);
            auto const value_variability = properties.value_variability(name);
            ...
            switch (value_variability)
            {
                ...
                {
                    auto const shape_variability = properties.shape_variability(name);
        ...
        py::object property_reference(Properties& properties, std::string const& name)
        {
            py::object property = py::none();
            auto const shape_per_object = properties.shape_per_object(name);
            auto const value_variability = properties.value_variability(name);
    ...
    TODO docstring
                ...
                .def_property_readonly(
                    ...
                    [](Properties const& properties)
                    {
                        ...
                        join_collections(properties.names<same_shape::Properties>(), names);
                        join_collections(properties.names<same_shape::constant_shape::Properties>(), names);
                        join_collections(properties.names<same_shape::variable_shape::Properties>(), names);
                        join_collections(properties.names<different_shape::Properties>(), names);
                        join_collections(
                            properties.names<different_shape::constant_shape::Properties>(), names);
                        join_collections(
                            properties.names<different_shape::variable_shape::Properties>(), names);
                ...
                .def(
                    "value_variability",
                    [](Properties const& properties, std::string const& name)
    ...
    Return whether the property-set has a time domain
                ...
                .def_property_readonly(
                    ...
                    [](PropertySet& property_set) -> TimeDomain&
                    {
                        ...
                        {
                            throw std::runtime_error("Property set does not have a time domain");
                        }
    ...
    TODO docstring
                ...
                .def(
                    "__getattr__",
                    [](PropertySet& property_set, std::string const& property_name)
                    {
                ...
                .def(
                    ...
                    {
                        auto const datatype_ =
                            numpy_type_to_memory_datatype(py::dtype::from_args(dtype_args));

                        return property_set.properties().add(name, datatype_, description);
                    },
                    "name"_a,
                    "dtype"_a,
                    "description"_a = "",
    ...
    :raises RuntimeError: In case the property cannot be created
                ...
                .def(
                       ...
                       std::string const& description) -> same_shape::Property&
                    {
                        ...
                        return property_set.properties().add(name, datatype_, shape_, description);
                    },
                    "name"_a,
                    "dtype"_a,
                    "shape"_a,
                ...
                .def(
                       ...
                       std::string const& description)
                    {
                        ...
                        switch (value_variability)
                        {
                            ...
                            {
                                using Property = same_shape::Property;
                                property = py::cast(&property_set.properties().add<Property>(
                                    name, datatype_, shape_, description));
                                break;
                            }
                            case ValueVariability::variable:
                            {
    ...
    :raises RuntimeError: In case the property cannot be created
                ...
                .def(
                       ...
                       std::string const& description) -> different_shape::Property&
                    {
                        ...
                        return property_set.properties().add(name, datatype_, rank, description);
                    },
                    "name"_a,
                    "dtype"_a,
                    "rank"_a,
                            ...
                            case ShapePerObject::same:
                            {
                                switch (shape_variability)
                                {
                                    ...
                                    case ShapeVariability::variable:
                                    {
                                        using Property = same_shape::variable_shape::Property;
                                        property = py::cast(&property_set.properties().add<Property>(
                                            name, datatype_, rank, description));
                                        break;
                                    }
                                }
