...
namespace lue {
    namespace data_model {
        namespace {
            ...
            static std::string formal_string_representation(ActiveSetIndex const& index)
            {
                return fmt::format("ActiveSetIndex(pathname='{}')", index.id().pathname());
            }


            static std::string informal_string_representation(ActiveSetIndex const& index)
            {
                return fmt::format(
                    "{}\n"
                    "   nr_indices: {}",
    ...
    yourself. :class:`ObjectTracker` instances provide one.
                ...
                .def(
                    "__repr__",
                    [](ActiveSetIndex const& index) { return formal_string_representation(index); })

                .def(
                    "__str__",
                    [](ActiveSetIndex const& index) { return informal_string_representation(index); })

                .def_property_readonly("nr_indices", &ActiveSetIndex::nr_indices)
