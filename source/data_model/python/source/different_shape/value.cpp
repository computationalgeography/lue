...
namespace lue {
    ...
    Value docstring...
                    ...
                    .def("__len__", &Value::nr_objects)

                    .def("__getitem__", &Value::operator[])
                    ...
                    .def(
                           ...
                           py::array_t<hdf5::Shape::value_type> const& shapes) -> Value&
                        {
                            if (ids.ndim() != 1)
                            {
                                throw std::runtime_error("IDs must be represented by a scalar value");
                            }

                            if (shapes.ndim() != 2)
                            {
                                throw std::runtime_error("Shapes must be represented by a 1D array");
                            }
                            ...
                            {
                                throw std::runtime_error("Number of IDs must equal number of shapes");
                            }
