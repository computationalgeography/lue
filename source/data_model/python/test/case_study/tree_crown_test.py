...
class TreeCrownTest(lue_test.TestCase):
    def test_case_study1(self):
        ...
        def add_stem_properties(phenomenon):
            ...
            space_points = np.arange(
                nr_trees * rank, dtype=space_coordinate_datatype
            ).reshape(nr_trees, 2)
            space_domain.value.expand(nr_trees)[:] = space_points

            # Property
            tree_kind = stem_points.add_property("kind", dtype=np.uint8)
            tree_kind.value.expand(nr_trees)[:] = (
                10 * np.random.rand(nr_trees)
            ).astype(np.uint8)
        ...
        def discretized_presence(property_set):
            ...
            shapes = np.arange(1, nr_trees * rank + 1, dtype=count_datatype).reshape(
                nr_trees, 2
            )
            ...
            for t in range(nr_trees):
                value_array = presence.value.expand(
                    ids[t], tuple(shapes[t]), nr_time_boxes
                )
                for b in range(nr_time_boxes):
                    values = (10 * np.random.rand(*shapes[t])).astype(presence_datatype)
        ...
        def add_crown_properties(phenomenon):
            ...
            crown_boxes.object_tracker.active_object_index.expand(
                ...
                np.arange(0, nr_time_boxes, dtype=ldm.dtype.Index), repeats=nr_trees
            )
            ...
            crown_boxes.object_tracker.active_object_id.expand(
                ...
                ids.reshape((1, nr_trees)), repeats=nr_time_boxes, axis=0
            ).flatten()
    ...
    def test_case_study2(self):
        ...
        time_cell_configuration = ldm.TimeConfiguration(ldm.TimeDomainItemType.cell)
        ...
        crowns_biomass_discretization = trees_globals.add_property(
            "biomass_discretization", dtype=count_datatype, shape=(space_rank,)
        )
        crowns_biomass_discretization.value.expand(1)[:] = np.array(
            [max_size_of_crown, max_size_of_crown], dtype=count_datatype
        ).reshape(1, space_rank)

        # Link biomass to discretization
        crowns_biomass.set_space_discretization(
            ldm.SpaceDiscretization.regular_grid, crowns_biomass_discretization
        )
        ...
        forest_shapes = (
            np.arange(nr_forests * space_rank, dtype=count_datatype).reshape(
                nr_forests, space_rank
            )
            + 10
        )
        forests_biomass_discretization.value.expand(nr_forests)[:] = np.array(
            [100, 100], dtype=count_datatype
        ).reshape(nr_forests, space_rank)

        # Link biomass to discretization
        areas_biomass.set_space_discretization(
            ldm.SpaceDiscretization.regular_grid, forests_biomass_discretization
        )
        ...
        crowns_time_cell_counts = crowns.time_domain.value.count.expand(nr_years)
        ...
        for y in range(nr_years):
            ...
            time_box = np.array([t_start, t_end], dtype=time_coordinate_datatype)
            ...
            for d in range(nr_days_per_growth_season):
                ...
                starting_tree_stem_locations = np.arange(
                    nr_starting_trees * space_rank, dtype=space_coordinate_datatype
                ).reshape(nr_starting_trees, space_rank)
                starting_tree_crown_locations = np.arange(
                    nr_starting_trees * space_rank * 2, dtype=space_coordinate_datatype
                ...
                crowns.space_domain.value.expand(nr_starting_trees)[
                    -nr_starting_trees
                ...
                for id_ in forest_ids:
                    ...
                    areas_biomass.value[id_][y] = np.arange(
                        nr_cells_in_forest, dtype=biomass_datatype
