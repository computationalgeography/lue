...
namespace lue {
    namespace hdf5 {
        ...
        unsigned int python_mode_to_hdf5_flag(std::string const& mode)
        {
            ...
            {
                throw py::value_error("mode string must begin with one of 'r' or 'w', not '" + mode + "'");
            }
        ...
        std::string intent_to_python_mode(unsigned int const intent)
        {
            std::string mode;
            ...
            {
                throw py::value_error("intent expected to be H5F_ACC_RDONLY or H5F_ACC_RDWR");
            }
