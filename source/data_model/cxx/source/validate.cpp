...
namespace lue::data_model {
    namespace {
        ...
        void not_supported_yet(hdf5::Identifier const& id, hdf5::Issues& issues, std::string const& message)
        {
            issues.add_warning(
                id,
                std::format(
    ...
    auto message(hdf5::Issues const& issues) -> std::string
    {
    ...
    static auto assert_ids_are_unique(hdf5::Identifier const& id, std::vector<ID> ids, hdf5::Issues& issues)
        -> bool
    {
        std::ranges::sort(ids);

        auto const it = std::ranges::adjacent_find(ids);
    ...
    auto assert_strictly_increasing(
        hdf5::Identifier const& id, Collection const& collection, hdf5::Issues& issues) -> bool
    {
        ...
        auto const it = std::adjacent_find(
            collection.begin(),
            collection.end(),
            [](Value const& lhs, Value const& rhs) -> auto { return lhs >= rhs; });
    ...
    template<typename Collection>
    auto assert_increasing(hdf5::Identifier const& id, Collection const& collection, hdf5::Issues& issues)
        -> bool
    {
        using Value = typename Collection::value_type;
        ...
        ObjectID const& object_id, different_shape::Value const& value, hdf5::Issues& issues)
    {
        ...
        else
        {
            ...
            for (auto const id : ids)
            {
                ...
                {
                    issues.add_error(
                        value.id(), std::format("For at least one object, no value is stored (id={})", id));
                }
            }
        }
    }
        ...
        hdf5::Issues& issues)
    {
        ...
        if (nr_value_arrays != nr_objects)
        {
            ...
            auto it = std::ranges::find_if(
                unique_object_ids, [&](auto const id) -> auto { return !value.exists(id); });
    ...
    static void validate_value(
        ObjectTracker const& object_tracker, same_shape::variable_shape::Value& value, hdf5::Issues& issues)
    {
        ...
        else
        {
            ...
            for (std::size_t i = 1; i < active_set_idxs.size(); ++i)
            {
                ...
                if (!assert_ids_are_unique(
                        active_object_id_id,
                        std::vector<ID>(
                            active_object_ids.begin() + static_cast<ssize_t>(begin_idx),
                            active_object_ids.begin() + static_cast<ssize_t>(end_idx)),
                        issues))
                {
                    break;
                }
    ...
    static void validate(ObjectTracker const& object_tracker, TimeDomain& time_domain, hdf5::Issues& issues)
    {
    ...
    static void validate(ObjectID const& object_id, StationarySpaceBox const& space_box, hdf5::Issues& issues)
    {
        ...
        hdf5::Issues& issues)
    {
        ...
        else
        {
            ...
            {
                validate_space_constant_regular_grid(discretized_property, discretization_property, issues);
            }
            ...
            if (discretization_value.array_shape() != hdf5::Shape{static_cast<Count>(property_value.rank())})
            {
                ...
                case TimeDiscretization::regular_grid:
                {
                    ...
                    if (values_can_be_tested)
                    {
                        auto const& discretization_value =
                            properties
                                .collection<
                                    same_shape::constant_shape::Properties>()[discretization_property_name]
                                .value();
                        ...
                        if (discretization_value.array_shape().size() != 1)
                        {
                            ...
                            discretization_value.array_shape()[0] != property_value.nr_locations_in_time())
                        {
                            issues.add_error(
                                discretization_property.id(),
                                std::format(
