...
class Fixture: public lue::hdf5::FileFixture
{
    ...
    private:
        ...
        std::unique_ptr<lue::data_model::different_shape::variable_shape::Value> _value;
};
...
BOOST_FIXTURE_TEST_CASE(update_all_object_arrays_per_location_in_time, Fixture)
{
    ...
    {
        auto t_value = value.expand(t, nr_active_objects[t], active_ids[t].data(), array_shapes[t].data());
        ...
        for (std::size_t o = 0; o < t_value.nr_objects(); ++o)
        {
            ...
            BOOST_CHECK(array.file_datatype() == lue::hdf5::file_datatype(datatype()));
