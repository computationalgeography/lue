    ...
    public:
        ...
        Fixture():
            ...
            _value{std::make_unique<lue::data_model::same_shape::Value>(
                lue::data_model::same_shape::create_value(*_file, _value_name, _datatype, _array_shape))}
        {
        }
...
BOOST_FIXTURE_TEST_CASE(expand, Fixture)
{
    auto& value = this->value();
    ...
    value.expand(nr_arrays);
    BOOST_CHECK_EQUAL(value.nr_arrays(), 2 * nr_arrays);
}
