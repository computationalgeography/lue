...
void init_space_point_domain_class(
    py::module& module)
{

    init_numpy();

    py::class_<SpacePointDomain, SpaceDomain>(
        module,
        "SpacePointDomain",
        "SpacePointDomain docstring...")

        .def(
            "reserve",
            &SpacePointDomain::reserve,
            "reserve docstring...",
            py::return_value_policy::reference_internal)

        .def_property_readonly(
            "items",
            py::overload_cast<>(&SpacePointDomain::items),
            "items docstring...",
            py::return_value_policy::reference_internal)
    ...
    module.def(
        "create_space_point_domain",
        [](
            PropertySet& property_set,
            py::handle const& numpy_type_id_object,
            size_t const rank) // -> SpacePointDomain
        {
            ...
            return create_space_point_domain(property_set,
                hdf5::Datatype(file_type_id), hdf5::Datatype(memory_type_id),
                rank);
        },
        R"(
    Create new space point domain
