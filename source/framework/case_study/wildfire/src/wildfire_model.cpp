...
namespace lue {
    namespace {
        ...
        lh5::Shape shape_to_shape(WildfireModel::RasterShape const& input_shape)
        {
    ...
    WildfireModel::WildfireModel(
        ldm::Count const nr_time_steps, RasterShape const& raster_shape, RasterShape const& partition_shape):
        ...
        _with_io{true},
        _dataset_ptr{std::make_shared<ldm::Dataset>(ldm::create_dataset(dataset_pathname))},
        _constant_raster_view{ldm::constant::create_raster_view(
            _dataset_ptr,
            phenomenon_name,
            constant_property_set_name,
            shape_to_shape(raster_shape),
            {0.0, 0.0, double(raster_shape[1]), double(raster_shape[0])})},
        _variable_raster_view{ldm::variable::create_raster_view(
            _dataset_ptr,
            phenomenon_name,
    ...
    void WildfireModel::simulate(Count const time_step)
    {
