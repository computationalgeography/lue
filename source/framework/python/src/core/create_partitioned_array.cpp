...
using Shape = std::vector<lue::Count>;

Shape tuple_to_shape(
    ...
    ShapeT<PartitionedArray<Element, rank>> const& partition_shape)
{
    ...
    return PartitionedArray<Element, rank>(array_shape, partition_shape);
}
...
template<
    Rank rank,
    typename Element>
std::array<Element, rank> dynamic_shape_to_static_shape(
    std::vector<Element> const& shape)
{
    lue_assert(shape.size() == rank);
    std::array<Element, rank> result{};
...
{
    // TODO Generalize for more ranks
    ...
    switch(kind)
    {
        case 'i':
        {
            ...
            {
                // case 1: {
                //     result = pybind11::cast(create_partitioned_array<std::int8_t, rank>(
                //         dynamic_shape_to_static_shape<rank>(array_shape),
                //         dynamic_shape_to_static_shape<rank>(partition_shape)));
                //     break;
                // }
                // case 2: {
                //     result = pybind11::cast(create_partitioned_array<std::int16_t, rank>(
                //         dynamic_shape_to_static_shape<rank>(array_shape),
                //         dynamic_shape_to_static_shape<rank>(partition_shape)));
                //     break;
                // }
        ...
        case 'u':
        {
            ...
            switch(size)
            {
                ...
                // case 2: {
                //     result = pybind11::cast(create_partitioned_array<std::uint16_t, rank>(
                //         dynamic_shape_to_static_shape<rank>(array_shape),
                //         dynamic_shape_to_static_shape<rank>(partition_shape)));
                //     break;
                // }
    ...
    module.def(
            ...
            pybind11::dtype const& dtype)
        {
            return create_partitioned_array(
                tuple_to_shape(array_shape), tuple_to_shape(partition_shape), dtype);
        },
