...
namespace lue::framework {
    namespace {
            ...
            DynamicShape const& partition_shape)
        {
            ...
            auto const [object_id, raster_shape, datatype] = ldm::constant::probe_raster(
            ...
            return read(constant::ArrayIO{hyperslab}, datatype,
            ...
            DynamicShape const& partition_shape)
        {
            auto const [dataset_pathname, phenomenon_name, property_set_name, layer_name] =
                parse_array_pathname(array_pathname);
            auto const [object_id, _, datatype] = ldm::constant::probe_raster(
                dataset_pathname, phenomenon_name, property_set_name, layer_name);
            ...
            Index const time_step_idx)
        {
            auto const [dataset_pathname, phenomenon_name, property_set_name, layer_name] =
                parse_array_pathname(array_pathname);
            auto const [object_id, _, datatype] = ldm::variable::probe_raster(
                dataset_pathname, phenomenon_name, property_set_name, layer_name);
    ...
    use the overload that accepts a time step index.
        ...
        module.def(
                ...
                pybind11::tuple const& partition_shape)
            {
                return read_array(
                    array_pathname,
                    tuple_to_shape(center_cell), tuple_to_shape(subset_shape), tuple_to_shape(partition_shape));
            },
            R"(
