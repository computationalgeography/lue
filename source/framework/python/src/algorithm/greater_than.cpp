...
#define LUE_GREATER_THAN_OVERLOADS(type, rank)  \
    module.def("greater_than", greater_than<type, std::uint8_t, rank, PartitionedArray<type, rank>, PartitionedArray<type, rank>>);  \
    module.def("greater_than", greater_than<type, std::uint8_t, rank, PartitionedArray<type, rank>, type>                        );  \
    module.def("greater_than", greater_than<type, std::uint8_t, rank, PartitionedArray<type, rank>, hpx::shared_future<type>>    );  \
    module.def("greater_than", greater_than<type, std::uint8_t, rank, type                        , PartitionedArray<type, rank>>);  \
    module.def("greater_than", greater_than<type, std::uint8_t, rank, hpx::shared_future<type>    , PartitionedArray<type, rank>>);
