...
Results will be written to the terminal if no output pathname is provided
            ...
            /*!
                @brief      Return an object representing the environment in which the
                            benchmark is run
            */
            Environment create_environment(std::map<std::string, docopt::value> const& arguments)
            {
            ...
            Task create_task(std::map<std::string, docopt::value> const& /* arguments */)
            {
        ...
        inline int run_benchmark(Benchmark& benchmark, std::ostream& stream)
        {
...
#define LUE_CONFIGURE_BENCHMARK()                                                                            \
    ...
    {                                                                                                        \
        std::map<std::string, docopt::value> arguments =                                                     \
            docopt::docopt(fmt::format(lue::benchmark::detail::usage, argv[0]), {argv + 1, argv + argc});    \
                                                                                                             \
        std::string const pathname = arguments["<output>"] ? arguments.at("<output>").asString() : "";       \
        auto const environment{lue::benchmark::detail::create_environment(arguments)};                       \
        auto const task{lue::benchmark::detail::create_task(arguments)};                                     \
                                                                                                             \
