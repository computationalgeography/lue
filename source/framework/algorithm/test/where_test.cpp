...
namespace {
        ...
        std::initializer_list<Element>&& result_values_we_want)
    {
        if constexpr (lue::BuildOptions::default_value_policies_enabled)
        {
            ...
            condition.partitions()(0, 0).set_data(hpx::launch::sync, std::move(condition_data));
            ...
            true_array.partitions()(0, 0).set_data(hpx::launch::sync, std::move(true_data));
            ...
            result_we_want.partitions()(0, 0).set_data(hpx::launch::sync, std::move(result_we_want_data));
        ...
        std::initializer_list<Element>&& result_values_we_want)
    {
        if constexpr (lue::BuildOptions::default_value_policies_enabled)
        {
            ...
            false_array.partitions()(0, 0).set_data(hpx::launch::sync, std::move(false_data));
...
BOOST_AUTO_TEST_CASE(use_case_1)
{
    ...
    if constexpr (lue::BuildOptions::default_policies_enabled)
    {
        ...
        Array<ConditionElement, rank> condition{
            lue::create_partitioned_array<ConditionElement>(shape, shape)};
        condition.partitions()(0, 0).get();
        condition.partitions()(0, 0).set_data(hpx::launch::sync, condition_data);
        ...
        Array<Element, rank> true_array{lue::create_partitioned_array<Element>(shape, shape)};
        true_array.partitions()(0, 0).get();
        true_array.partitions()(0, 0).set_data(hpx::launch::sync, true_data);
        ...
        Array<Element, rank> result_we_want{lue::create_partitioned_array<Element>(shape, shape)};
        result_we_want.partitions()(0, 0).get();
        result_we_want.partitions()(0, 0).set_data(hpx::launch::sync, result_we_want_data);
