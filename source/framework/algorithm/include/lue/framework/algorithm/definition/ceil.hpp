...
namespace lue {
    ...
    PartitionedArray<Element, rank> ceil(
        Policies const& policies, PartitionedArray<Element, rank> const& array)
    {
        return unary_local_operation(policies, array, detail::Ceil<Element>{});
    }

}  // namespace lue


#define LUE_INSTANTIATE_CEIL(Policies, Element, rank)                                                        \
                                                                                                             \
    template LUE_LOCAL_OPERATION_EXPORT PartitionedArray<Element, rank>                                      \
    ceil<ArgumentType<void(Policies)>, Element, rank>(                                                       \
