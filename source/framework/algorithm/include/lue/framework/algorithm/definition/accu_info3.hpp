    ...
    namespace detail {
        ...
        class AccuInfoCellAccumulator
        {
            ...
            public:
                ...
                AccuInfoCellAccumulator(Policies const& policies, CellClassData& cell_class):
                        ...
                        hpx::future<CellClassData>&& cell_class_data_f) mutable
                    {
                        ...
                        auto accumulate =
                            [&accu_mutex,
                             ...
                             &inflow_count_data,
                             &cell_class_data](
                                std::array<Index, rank> const& cell_idxs, CellClass const value) mutable
                        {
                            ...
                            if (inflow_count_data(idx0, idx1) == 0)
                            {
                                ...
                                {
                                    auto const [exit_cell_idx0, exit_cell_idx1] = exit_cell_idxs;
                                    cell_class_data(exit_cell_idx0, exit_cell_idx1) = partition_output_cell;
                                }
                            }
                        };
            ...
            hpx::future<CellClassPartition> cell_class_partition_f = hpx::dataflow(
                   ...
                   hpx::future<CellClassData>&& cell_class_data_f)
                {
                    ...
                    return CellClassPartition{
                        hpx::new_<Server>(hpx::find_here(), partition_offset, cell_class_data_f.get())};
                },
        ...
        struct AccuInfoAction3:
            hpx::actions::make_action<
                decltype(&accu_info3_partition<Policies, FlowDirectionElement, rank>),
                &accu_info3_partition<Policies, FlowDirectionElement, rank>,
                AccuInfoAction3<Policies, FlowDirectionElement, rank>>::type
        {
        };
        ...
        -> PartitionedArray<CellClass, rank>
    {
        ...
        for (Index p = 0; p < nr_partitions; ++p)
        {
            cell_class_partitions[p] = hpx::async(
                ...
                cell_class_communicators[p]);
        }


        hpx::when_all(
            cell_class_partitions.begin(),
            cell_class_partitions.end(),
            [inflow_count_communicators = std::move(inflow_count_communicators),
             cell_class_communicators =
                 std::move(cell_class_communicators)]([[maybe_unused]] auto&& partitions) mutable
            {
...
#define LUE_INSTANTIATE_ACCU_INFO3(Policies, FlowDirectionElement)                                           \
                                                                                                             \
    template LUE_ROUTING_OPERATION_EXPORT PartitionedArray<CellClass, 2>                                     \
    accu_info3<ArgumentType<void(Policies)>, FlowDirectionElement, 2>(                                       \
