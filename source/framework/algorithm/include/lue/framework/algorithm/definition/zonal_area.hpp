#pragma once
#include "lue/framework/algorithm/zonal_area.hpp"
#include "lue/framework/algorithm/definition/zonal_sum.hpp"


namespace lue {
    // namespace detail {

    //     template<
    //         typename Count,
    //         typename Zone>
    //     class ZonalArea
    //     {

    //         public:

    //             static_assert(std::is_arithmetic_v<Count>);

    //             static_assert(std::is_integral_v<Zone>);

    //             using OutputElement = Count;


    //             class Aggregator
    //             {

    //                 public:

    //                     using Map = std::unordered_map<Zone, OutputElement>;


    //                     void add(
    //                         Zone const zone,
    //                         InputElement const value)
    //                     {
    //                         if(_area_by_zone.find(zone) == _area_by_zone.end())
    //                         {
    //                             _area_by_zone[zone] = value;
    //                         }
    //                         else
    //                         {
    //                             _area_by_zone[zone] += value;
    //                         }
    //                     }


    //                     void merge(
    //                         Aggregator const& other)
    //                     {
    //                         for(auto const& [zone, value]: other._area_by_zone)
    //                         {
    //                             add(zone, value);
    //                         }
    //                     }


    //                     bool contains(
    //                         Zone const zone) const
    //                     {
    //                         return _area_by_zone.find(zone) != _area_by_zone.end();
    //                     }


    //                     InputElement operator[](
    //                         Zone const zone) const
    //                     {
    //                         auto it{_area_by_zone.find(zone)};

    //                         lue_hpx_assert(it != _area_by_zone.end());

    //                         return (*it).second;
    //                     }


    //                 private:

    //                     friend class hpx::serialization::access;


    //                     template<
    //                         typename Archive>
    //                     void serialize(
    //                         Archive& archive,
    //                         unsigned int const /* version */)
    //                     {
    //                         archive & _area_by_zone;
    //                     }


    //                     Map _area_by_zone;

    //             };

    //     };

    // }  // namespace detail


    template<
        typename Count,
        typename Policies,
        typename Zone,
        Rank rank>
    PartitionedArray<Count, rank> zonal_area(
        Policies const& policies,
        PartitionedArray<Zone, rank> const& zones)
    {
        using Functor = detail::ZonalSum<Count, Zone>;

        return zonal_operation(policies, Count{1}, zones, Functor{});
    }

}  // namespace lue


#define LUE_INSTANTIATE_ZONAL_AREA(                         \
    Policies, Count, Zone)                                  \
                                                            \
    template LUE_ZONAL_OPERATION_EXPORT                     \
    PartitionedArray<Count, 2> zonal_area<                  \
            Count, ArgumentType<void(Policies)>, Zone, 2>(  \
        ArgumentType<void(Policies)> const&,                \
        PartitionedArray<Zone, 2> const&);
