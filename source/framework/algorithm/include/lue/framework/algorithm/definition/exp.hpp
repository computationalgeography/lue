...
namespace lue {
    ...
    {
        return unary_local_operation(policies, array, detail::Exp<Element>{});
    }

}  // namespace lue


#define LUE_INSTANTIATE_EXP(Policies, Element, rank)                                   \
                                                                                       \
    template LUE_LOCAL_OPERATION_EXPORT                                                \
    PartitionedArray<Element, rank> exp<ArgumentType<void(Policies)>, Element, rank>(  \
