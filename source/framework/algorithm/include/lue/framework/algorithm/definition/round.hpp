...
namespace lue {
    ...
    {
        return unary_local_operation(policies, array, detail::Round<Element>{});
    }

}  // namespace lue


#define LUE_INSTANTIATE_ROUND(Policies, Element, rank)                                                       \
                                                                                                             \
    template LUE_LOCAL_OPERATION_EXPORT PartitionedArray<Element, rank>                                      \
    round<ArgumentType<void(Policies)>, Element, rank>(                                                      \
