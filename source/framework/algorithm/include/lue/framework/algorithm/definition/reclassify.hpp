...
namespace lue {
    namespace detail {
        ...
        OutputPartition reclassify_partition(
            Policies const& policies,
            InputPartition const& input_partition,
            hpx::shared_future<LookupTable<ElementT<InputPartition>, ElementT<OutputPartition>>> const& lookup_table)
        {
            using FromElement = ElementT<InputPartition>;
        ...
        hpx::shared_future<LookupTable<FromElement, ToElement>> const& lookup_table)
    {
        ...
        for(Index p = 0; p < nr_partitions(input_array); ++p)
        {
            output_partitions[p] = hpx::async(
                action, localities[p], policies, input_partitions[p], lookup_table);
        }
...
#define LUE_INSTANTIATE_RECLASSIFY(                                    \
    Policies, FromElement, ToElement)                                  \
                                                                       \
    template LUE_GLOBAL_OPERATION_EXPORT                               \
    PartitionedArray<ToElement, 2> reclassify<                         \
            ArgumentType<void(Policies)>, FromElement, ToElement, 2>(  \
