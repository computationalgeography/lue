...
namespace lue {
    namespace detail {
            ...
            MaterialCommunicator<MaterialElement, rank> material_communicator)
        {
                    ...
                    hpx::split_future(
                        hpx::dataflow(
                                    ...
                                    output_cells_idxs_f) mutable
                            {
                                ...
                                auto const threshold_partition_ptr{ready_component_ptr(threshold_partition)};
                                MaterialData const& threshold_data{threshold_partition_ptr->data()};
            ...
            // Solve inter-partition stream cells
            {
                ...
                hpx::tie(outflow_data_f, remainder_data_f) = hpx::split_future(
                    hpx::dataflow(
                        hpx::launch::async,
                        ...
                        [policies, material_communicator = std::move(material_communicator)](
                            ...
                            MaterialPartition const& threshold_partition,
                            hpx::future<InflowCountData>&& inflow_count_data_f,
                            hpx::shared_future<std::array<CellsIdxs, nr_neighbours<rank>()>>&&
                                input_cells_idxs_f,
                                    ...
                                    results.push_back(
                                        hpx::async(
                                            monitor_material_inputs<
                                                ...
                                                rank>,
                                            std::move(cells_idxs),
                                            material_communicator.receive_channel(
                                                accu::Direction::north_west),
            ...
            auto [outflow_partition_f, remainder_partition_f] = hpx::split_future(
                hpx::dataflow(
                    hpx::launch::async,
                       ...
                       hpx::future<MaterialData>&& remainder_data_f)
                    {
                        ...
                        return hpx::make_tuple(
                            ...
                            MaterialPartition{hpx::new_<Server>(
                                hpx::find_here(), partition_offset, remainder_data_f.get())});
                    },
        ...
        struct AccuThresholdAction3:
            hpx::actions::make_action<
                decltype(&accu_threshold3_partition<Policies, FlowDirectionElement, MaterialElement, rank>),
                &accu_threshold3_partition<Policies, FlowDirectionElement, MaterialElement, rank>,
                AccuThresholdAction3<Policies, FlowDirectionElement, MaterialElement, rank>>::type
        {
        };
        ...
        -> std::tuple<PartitionedArray<MaterialElement, rank>, PartitionedArray<MaterialElement, rank>>
    {
        ...
        {
            hpx::tie(outflow_partitions[p], remainder_partitions[p]) = hpx::split_future(
                hpx::async(
        ...
        return std::make_tuple(
            MaterialArray{
                flow_direction.shape(), Localities<rank>{localities}, std::move(outflow_partitions)},
            MaterialArray{
                flow_direction.shape(), Localities<rank>{localities}, std::move(remainder_partitions)});
    }

}  // namespace lue


#define LUE_INSTANTIATE_ACCU_THRESHOLD3(Policies, FlowDirectionElement, MaterialElement)                     \
                                                                                                             \
    template LUE_ROUTING_OPERATION_EXPORT                                                                    \
        std::tuple<PartitionedArray<MaterialElement, 2>, PartitionedArray<MaterialElement, 2>>               \
        accu_threshold3<ArgumentType<void(Policies)>, FlowDirectionElement, MaterialElement, 2>(             \
