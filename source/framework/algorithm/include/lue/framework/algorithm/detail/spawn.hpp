...
namespace lue::detail {
        ...
        ArrayPartitionData<Component, rank>&& array_of_components)
    {
        ...
        return hpx::when_all(array_of_components.begin(), array_of_components.end())
            .then(
                [shape = array_of_components.shape()](hpx::future<std::vector<Component>>&& f)
                {
                    std::vector<Component> vector_of_ready_components{f.get()};
                    ArrayPartitionData<Component, rank> array_of_ready_components{{shape}};

                    std::move(
                        vector_of_ready_components.begin(),
                        vector_of_ready_components.end(),
    ...
    template<typename... OutputComponent, typename Policies, typename Action, typename... InputComponents>
        ...
        InputComponents&&... input_components)
    {
        ...
        auto result = hpx::dataflow(
            ...
            hpx::unwrapping(
                ...
                [locality_id, action, policies, component_offset](InputComponents&&... input_components)
                { return action(locality_id, policies, component_offset, std::move(input_components)...); }
    ...
    template<typename ComponentCollection, typename... Index>
    ComponentT<ComponentCollection>& component_ref(
        ComponentCollection& components, Index const... component_idxs)
    {
    ...
    template<typename... ComponentCollections, std::size_t... collection_idxs, typename... Index>
    ...
    template<typename... ComponentCollections, typename... Index>
    hpx::tuple<ComponentT<ComponentCollections>&...> component_refs(
        std::tuple<ComponentCollections...>& components, Index const... component_idxs)
    {
        // Given a number of collections of components and component indices,
        // return a tuple with for each of the component collections a
        // reference to the component pointed to by those component indices
        ...
        InputComponentCollection const&... input_component_collection)
    {
        ...
        auto output_components{
            std::make_tuple<OutputComponentCollection...>(OutputComponentCollection{shape_in_components}...)};
        ...
        component_refs(output_components, idx0, idx1) =
            spawn_component<ComponentT<OutputComponentCollection>...>(
