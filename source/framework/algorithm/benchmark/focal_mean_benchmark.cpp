...
namespace lue {
    namespace benchmark {
        ...
        class FocalMeanBenchmarkModel final: public BenchmarkModel<Element, rank>
        {
            ...
            public:
                ...
                FocalMeanBenchmarkModel(Task const& task, std::size_t max_tree_depth);

                FocalMeanBenchmarkModel(FocalMeanBenchmarkModel const&) = default;

                FocalMeanBenchmarkModel(FocalMeanBenchmarkModel&&) = default;

                ~FocalMeanBenchmarkModel() = default;

                FocalMeanBenchmarkModel& operator=(FocalMeanBenchmarkModel const&) = default;

                FocalMeanBenchmarkModel& operator=(FocalMeanBenchmarkModel&&) = default;
        ...
        FocalMeanBenchmarkModel<Element, rank>::FocalMeanBenchmarkModel(
            Task const& task, std::size_t const max_tree_depth):
        ...
        {
            this->state() =
                value_policies::uniform(this->state(), Element{0}, std::numeric_limits<Element>::max());
            _kernel = lue::box_kernel<std::uint8_t, rank>(1, true);
        }


        template<typename Element, std::size_t rank>
        void FocalMeanBenchmarkModel<Element, rank>::do_simulate(Count const /* time_step */)
        {
