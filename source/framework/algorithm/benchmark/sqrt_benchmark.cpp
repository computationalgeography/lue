...
namespace lue {
    namespace benchmark {
        ...
        class SqrtBenchmarkModel final: public BenchmarkModel<Element, rank>
        {

            public:

                SqrtBenchmarkModel(Task const& task, std::size_t max_tree_depth);
                ...
                SqrtBenchmarkModel& operator=(SqrtBenchmarkModel const&) = default;

                SqrtBenchmarkModel& operator=(SqrtBenchmarkModel&&) = default;
        ...
        {
            this->state() =
                default_policies::uniform(this->state(), Element{0}, std::numeric_limits<Element>::max());
        }


        template<typename Element, std::size_t rank>
        void SqrtBenchmarkModel<Element, rank>::do_simulate(Count const /* time_step */)
        {
